...
const uint8_t PwmLev[9] = { 50, 65, 70, 75, 80, 85, 90, 95, 120 };   //占空比档位
...
void TIM16_PWM_Config(void) {
    ...
    TIMPeriodReload(TIM16, QIFAPWMHZ);
    ...
    TIM_TimeBaseStructure.TIM_Prescaler = PWM_CLK_DIV - 1;
...
void TIMPeriodReload(TIM_TypeDef *TIMx, uint32_t PwmHz) {
    PwmPra.TimerPeriod = (SystemCoreClock / (PWM_CLK_DIV * PwmHz)) - 1;
...
//PWM_PLUS OFF
void PWM_Plus_OFF(void) {
    TIM_CtrlPWMOutputs(TIM16, DISABLE);
    TIM_Cmd(TIM16, DISABLE);
    PwmPra.PWM_work_state = 0;
}



//PWM plus ON
void PWM_Plus_On(void) {
    TIM_SetCounter(TIM16, 0);
    TIMPeriodReload(TIM16, QIFAPWMHZ);
    TIMSetPwmValue(TIM16, QIFAPWMVALUE);         //占空比
    TIM_ARRPreloadConfig(TIM16, ENABLE);
    TIM_CtrlPWMOutputs(TIM16, ENABLE);
    TIM_Cmd(TIM16, ENABLE);
    PwmPra.PWM_work_state = 1;   //置标志 表示运行中
}

//PWM PLUS hz 设定
void Set_PWM_Plus_Hz(uint16_t PwmHz) {
    if (PwmPra.PWM_work_state) {
